...
# 初始化日志配置
logger = setup_logger('video_clip')

root_directory = path_utils.get_project_root()      
logger.info("当前应用根目录: %s", root_directory)
...
def get_transcription(file_url, timeout=60):
    ...
    if transcribe_response.status_code == HTTPStatus.OK:
        ...
        logger.info('transcription done! %s', response_str)
        # response_str转成json
        ...
        logger.error("Failed to get transcription: %s", transcribe_response.status_code)
...
def fetch_transcription_content(transcription_url):
    ...
    logger.debug("fetch_transcription_content>>> %s", fetch_transcription_content)
...
def download_vedio(url):
    ...
    parameters = {"url": url}
    payload = {
        "workflow_id": "7437454680492359706",
    ...
    if response.status_code == 200:
        ...
        logger.info("成功运行工作流！响应数据：%s", json.dumps(response_data, ensure_ascii=False))
        response_data_json = json.loads(response_data)
        return response_data_json["output"]
    else:
        logger.error("请求失败，状态码: %d，响应内容: %s", response.status_code, response.text)
        return ""
...
def get_images_from_unsplash(query, count=1):
    ...
    if response.status_code == 200:
        ...
        if data['results']:
            ...
            logger.warning("未找到与查询词 '%s' 相关的图片。", query)
            return []
    else:
...
def download_unsplash_image(download_location, filename):
    ...
    if response.status_code != 200:
        logger.error("Error getting download URL: %s", response.status_code)
        return
    ...
    try:
        ...
        logger.info("下载图片：%s", actual_image_url)
        image_response = requests.get(actual_image_url)
        if image_response.status_code == 200:
            with open(filename, 'wb') as f:
                f.write(image_response.content)
            logger.info("Image %s downloaded successfully.", filename)
        else:
            logger.error("Error downloading image %s: %s", filename, image_response.status_code)
            
    except Exception as e:
        logger.error("Error processing download: %s", str(e))
...
def download_image(url, clip_tmp_dir):
    ...
    if response.status_code == 200:
        ...
        logger.info("Download Image from %s to %s successfully.", url, filename)
        return filename
    else:
        logger.error("Error downloading Image from %s: %s", url, response.status_code)
        return ""
...
def extract_audio_from_url(url, clip_tmp_dir="clip_tmp_" + str(time.time()) ):
    ...
    try:
        ...
        logger.info(f"下载文件成功，开始保存到本地：{media_temp_path}")
        # 使用流式下载
        with open(media_temp_path, 'wb') as f:
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    f.write(chunk)
        logger.info(f"下载文件成功，保存到本地：{media_temp_path}")
        # 尝试作为视频文件加载
        try:
            ...
            video.close()
            logger.info(f"创建视频剪辑成功，视频路径：{media_temp_path}, 提取音频路径：{audio_temp_path}")
        except Exception as e:
            logger.error(f"视频处理失败，尝试作为音频文件处理: {str(e)}")
            # 如果不是视频文件，直接作为音频文使用
        ...
        try:
            ...
            logger.info(f"ffmpeg -i {audio_temp_path} {wav_temp_path} success!!!")
...
def create_video(title, cover_image_path, caption_list, audio_path, clip_tmp_dir, bg_music_path=None):
    ...
    logger.info(f"预览图已保存到: {preview_image_path}")
...
def create_video_with_captions(title, caption_list, audio_url, bg_music_path=None):
    ...
    try:
        ...
        logger.info("视频制作参数：\n标题：%s\n封面图片路径：%s\n音频路径：%s\n临时目录：%s", 
                   title, cover_image_path, audio_path, clip_tmp_dir)
        logger.debug("字幕列表：%s", json.dumps(caption_list, ensure_ascii=False))
        
        # 5. 制作视频
        output_path, preview_image_path = create_video(
            title, cover_image_path[0], caption_list, audio_path, clip_tmp_dir, bg_music_path
        )
        return output_path, preview_image_path
    except Exception as e:
        logger.error("视频制作失败：%s", str(e))
        raise
    finally:
        # 清理临时目录的注释也改为logger
        logger.debug("保留临时目录用于调试：%s", clip_tmp_dir)


if __name__ == '__main__':
    url = input("请输入一个视频URL: ")
    logger.info("处理视频URL: %s", url)
    
    bg_music_choice = input("是否需要背景音乐？(y/n): ")
    if bg_music_choice == "y":
        bg_music_path = os.path.join(root_directory, "static", "bg_music.mp3")
        logger.info("使用背景音乐: %s", bg_music_path)
    else:
        bg_music_path = None
        logger.info("不使用背景音乐")
    
    vedeo_info = download_vedio(url)
    logger.info("获取到视频信息: %s", vedeo_info)
    
    audio_url = vedeo_info["url"]
    title = vedeo_info["title"]
    
    logger.info("开始获取字幕，标题: %s", title)
